...
//            break;
        ...
        case 3:
            // wait
           asm("  MRS R0, PSP");
           current_semaphore = getR0();
           if (current_semaphore->count > 0)
               current_semaphore->count--;
           else
           {
               current_semaphore->queueSize++;
               current_semaphore->processQueue[current_semaphore->queueSize] = taskCurrent;
               tcb[taskCurrent].state = STATE_BLOCKED;
               tcb[taskCurrent].semaphore = current_semaphore;
               NVIC_INT_CTRL_R |= NVIC_INT_CTRL_PEND_SV;
           }
           break;
        ...
        case 4:
           ...
           current_semaphore = getR0();
           current_semaphore->count++;
           if (current_semaphore->queueSize > 0)
           {
               current_semaphore->count--;
               waiting_task = current_semaphore->processQueue[0];
               tcb[waiting_task].state = STATE_READY;
               uint8_t j;
               for (j = 0; j < current_semaphore->queueSize; j++)
                   current_semaphore->processQueue[j] = current_semaphore->processQueue[j+1];
               current_semaphore->queueSize--;
           }
           break;
